...
class Admin extends Controller
{
    ...
    {
        $this->model('Produk_model')->hapusDataProduk($id);
        Flasher::setFlash('Produk', 'dihapus', 'success');
        header('Location: ' . BASEURL . '/Admin/produk');
        exit;
    }
    public function hapusPromo($id)
    {
        $this->model('Promo_model')->hapusDataPromo($id);
        Flasher::setFlash('Promo', 'dihapus', 'success');
        header('Location: ' . BASEURL . '/Admin/promo');
        exit;
    }
    ...
    public function getubahPromo()
    {
        echo json_encode($this->model('Promo_model')->getPromoById($_POST['id']));
    }
    public function users()
    {
        $data['users'] = [['id' => 1, 'nama' => 'Bagas Al Fakhri', 'email' => 'bagas@gmail.com', 'status' => 'Active']];
